...
def build_parser(*, manage_workflow, manage_sessions, display_stitch_info):
    description = (
        ...
        'Examples:'
        '\n'
        "      amscrot workflow --var-file vars.yml --session test-chi -validate"
        '\n'
        "      amscrot workflow --config-dir . --session test-chi -validate"
        '\n'
        '      amscrot stitch-policy -providers "fabric,sense"'
    )

    parser = create_parser(description=description)
    subparsers = parser.add_subparsers()
    workflow_parser = subparsers.add_parser('workflow', help='Manage AmSCROT workflows')
    ...
    sessions_parser = subparsers.add_parser('sessions', help='Manage AmSCROT sessions ')
    ...
    stitch_parser.add_argument('-c', '--credential-file', type=str, default='~/.amscrot/amscrot_credentials.yml',
                               help='AmSCROT credential file. Defaults to ~/.amscrot/amscrot_credentials.yml',
                               required=False)
...
def get_log_level():
    import os
    return os.environ.get('AMSCROT_LOG_LEVEL', "INFO")


def get_log_location():
    import os
    return os.environ.get('AMSCROT_LOG_LOCATION', "/tmp/amscrot.log")
...
def disable_stdout_logs():
    import os
    return os.environ.get('AMSCROT_DISABLE_LOGS_TO_STDOUT', "false").lower() == 'true'
...
def get_stats_base_dir(friendly_name):
    ...
    base_dir = os.path.join(str(Path.home()), '.amscrot', 'stats', friendly_name)
...
def get_base_dir(friendly_name):
    ...
    base_dir = os.path.join(str(Path.home()), '.amscrot', 'sessions', friendly_name)
    os.makedirs(base_dir, exist_ok=True)
    return base_dir
...
def generate_bgp_key_if_needed(friendly_name):
    from amscrot.model.state import get_loader, get_dumper
    import yaml
    import os
